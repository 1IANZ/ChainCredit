    ...
    <Box sx={{ display: "flex", flexDirection: "column", height: "calc(100vh - 64px)", overflow: "hidden" }}>
      <Box sx={{ flexGrow: 1, display: "flex", alignItems: "center", justifyContent: "center", backgroundColor: "background.default", p: 2 }}>
        <Paper elevation={4} sx={{ p: { xs: 3, sm: 4 }, display: "flex", flexDirection: "column", alignItems: "center", width: "100%", maxWidth: 450, borderRadius: 3, boxSizing: "border-box" }}>
          ...
          <Typography variant="body1" color="text.secondary" sx={{ mb: 4, textAlign: "center" }}>Access your account with your private key.</Typography>
          <TextField label="Private Key" variant="outlined" type="password" fullWidth sx={{ mb: 3 }} value={privateKey} onChange={(e) => setPrivateKey(e.target.value)} />
          <Button variant="contained" color="primary" fullWidth sx={{ py: 1.5, fontWeight: "bold", fontSize: "1rem", textTransform: "none", mt: 2, "&:hover": { boxShadow: `0 5px 15px ${theme.palette.primary.main}60` } }} onClick={handleLogin}>
            私钥登录
          </Button>
        </Paper>
      </Box>
